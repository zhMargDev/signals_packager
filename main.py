import psycopg2, json, ccxt, asyncio, signals_packaging, traceback
...
async def get_folder_region(signal_channel_id, cursor):
    ...
    if len(channels) != 0:
        ...
        if len(folders) != 0:
            folder = folders[0]
            for key in fr.REGIONS:
                if folder[2] in fr.REGIONS[key]:
                    return key
            else:
                return 'RU'
        else:
...
async def defecte_signals(signal_id, conn, cursor, message):
    sql = 'INSERT INTO defective_signals (signal_id , message) VALUES (%s, %s)'
...
async def main():
    ...
    while True:
        ...
        try:    
            ...
            # Проверка существует ли такой сигнал в проверенных сигналах
                def_signals = f"SELECT * FROM defective_signals WHERE signal_id = {int(signal[0])}"
            ...
            # Праверка Стоп Сигнала , если он есть , то сравнивается с Таргетом , TVH или LVH
                ...
                if 'Def' not in stop_loss: 
                    ...
                    stop_loss_str_fixed = stop_loss.replace(',', '.')

                    # Конвертируем в float
                    stop_loss = float(stop_loss_str_fixed)
            ...
            # Проверка Таргетов, если таргет не пустой , Сравнивается процентный перепад             
                ...
                add_to_db = "INSERT INTO verified_signals (channel_id, message_id, channel_name, date, time, coin, trend, tvh, rvh, lvh, targets, stop_loss, leverage, margin) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)"
            ...
            # Выводим информацию о месте ошибки
            print(f"Информация об ошибке:\n{traceback_info}")
